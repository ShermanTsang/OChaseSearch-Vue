...
<template>
  <div class="search">
    <div class="search__box" :class="{'search__box--focused':state.showToolbox || state.isMouseOnToolbox}">
    ...
    <div v-if="state.showToolbox || state.isMouseOnToolbox" class="search__toolbox"
    ...
    >
      <Tab :tabNames="[$t('search.promptKeyword'),$t('search.historyKeyword')]"
           ...
           @changeTab="(tabName)=>{state.activeTab = tabName}">
        <div :slot="$t('search.promptKeyword')">
          <div v-if="data.promptKeywordList && data.promptKeywordList.length > 0"
            ...
            <div v-for="(keyword,index) in data.promptKeywordList" :key="keyword"
                 ...
                 :class="{'search__toolbox__tab__prompt__item--active': state.selectedPromptIndex === index}"
  ...
  export default {
    ...
    methods: {
      search(keyword = this.form.keyword.replace(/^\s+|\s+$/g, '')) {
        ...
        const isSearchedByPrompt = this.state.isInputFocused && this.state.selectedPromptIndex !== -1
        if (isSearchedByPrompt) {
          this.form.keyword = this.data.promptKeywordList[this.state.selectedPromptIndex]
        } else {
      ...
      selectItem(direction) {
        const itemIndex = this.state.selectedPromptIndex
        const currentPromptKeywordIndex = (this.data.promptKeywordList.length - 1)
        if (direction === 'down') {
          const isLastItem = itemIndex === currentPromptKeywordIndex
          this.state.selectedPromptIndex = isLastItem ? 0 : itemIndex + 1
        }
        if (direction === 'up') {
          const isFirstItem = itemIndex === -1
          this.state.selectedPromptIndex = isFirstItem ? currentPromptKeywordIndex : itemIndex - 1
        }
      },
